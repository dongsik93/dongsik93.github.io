...
- `html`화면에 띄우기
  - {% raw %}`{{message}}`{% endraw %} 의 위치는 연결을 시켜놓았던 `div`을 벗어나면 사용할 수 없다
...
```javascript
<div id="app4">
        <h3 v-for="todo in todos">{% raw %}{{todo}}{% endraw %}</h3>
</div>
...
```javascript
<div id="app">
        <h1 v-show="visible">{% raw %}{{message}}{% endraw %}</h1>
...
<ul>
	...
	<!-- 인덱스 값이 필요할 때 -->
    <li v-for="(todo, index) in todos">{% raw %}{{index}}. {{todo.text}}{% endraw %}</li>
</ul>
const app = new Vue({
        el:"#app",
        data:{
